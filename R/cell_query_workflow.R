...
fetch_representative_tiff_images <- function(
  ...
  image_res <- lapply(sample_ids, function(sample_id) {
    ...
    lapply(rects, function(r) {
      r_fetch <- TiffPlotR::rect_resize_mult(r, fetch_resize_mult)
      img_res <- TiffPlotR::fetchTiffData.rgb(img_file, r_fetch, blue_channel = 1, red_channel = 2, green_channel = 3)
